import { Question, Screen,TestLevel } from "./types";
...
export default function QuestionTable({ questions,testLevels, screens, onEdit, onDelete }: QuestionTableProps) {
  return (
    <div className="mb-6">
        ...
        <tbody>
          {questions.map(q => (
            <tr key={q.id} className="bg-[var(--card-bg)]">
              ...
              <td className="border border-[var(--border-color)] p-2 text-[var(--foreground)] text-center">{screens.find(s => s.id === q.screenId)?.screenNumber}(Level {testLevels.find(tl => tl.id === screens.find(s => s.id === q.screenId)?.testLevelId)?.level}) </td>
